...
function generarTablero(ancho, alto) {
    ...
    for (let i = 0; i < ancho * alto; i++) {                        //recorre todas las celdas del tablero. ejecuta  ancho * alto veces para crear todas las celdas
        ...
        newCelda.dataset.fila = Math.floor(i / ancho)               //se crean los atributos (atributos data, estableciendose con dataset) para rastrear la fila. se calcula dividiendo (i) entre el ancho del tablero (ancho), obteniendo el número de fila
        newCelda.dataset.col = i % ancho                            //para rastrear la columna. se calcula utilizando el operador % dividiendo (i) por el ancho (ancho) y toma el resto, dando el número de columna
...
function calcularMinasAlrededor(f, c) {        //fila y columna                         //calcula cuantas minas hay alrededor de una celda específica. recorre las celdas y cuenta las minas que encuentre
    ...
    for (let i = f-1; i <= f+1; i++) {                                                  //recorrer FILAS. recorrer las celdas a la celda en la fila (f) y columna (c). este bucle recorre las filas a circundantes a la fila (f). Comienza una fila antes de la fila de la celda (f-1), continúa hasta una fila después de la fila de la celda (f+1)
        for (let j = c-1; j <= c+1; j++) {                                              //recorre las COLUMNAS circundanetes a la columna (c). Comienza una columna antes de la columna de la celda (c-1), continúa hasta una columna después de la columna de la celda (c+1)
