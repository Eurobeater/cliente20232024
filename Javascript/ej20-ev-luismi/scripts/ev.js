...
function imprimirListado(listado) {
    ...
    listado.forEach( r => {                                                                 //antes listado era json
...
busqueda.addEventListener("keyup", function(ev)  {
    ...
    if (resultados.length && resultados[0].nombre) {
        filtrados = resultados.filter( r => r.nombre.toLowerCase().includes(this.value.trim().toLowerCase()))       //el filtro se guarda en un nuevo array, por eso hay que hacerlo en una variable    
    }else {
        filtrados = resultados.filter( r => quitarAcentos(r.text.toLowerCase().includes(this.value.trim().toLowerCase())))
    }
