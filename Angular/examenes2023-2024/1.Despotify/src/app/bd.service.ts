...
export class BdService {
  ...
  topSongByLength() {
    ...
    this.canciones.forEach((c: any) => {
      if (!topSong || c.length > topSong.length) {
        topSong = c;
      }
    });
    return topSong;
  ...
  promedioArtistas() {
    return this.playlists.map((playlist:any) =>
      playlist.songs.length).reduce((a:any, b:any) =>
        a + b) / this.playlists.length
  }

  isAlreadyInPlaylist(cancion:any) {
    return this.playlists[3].songs.includes(cancion.id)
  }
}
